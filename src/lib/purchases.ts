...
export async function getPurchasesForSeller(creatorId: string): Promise<Purchase[]> {
  const db = getDb();
  if (!db) return [];

  const colRef = collection(db, PURCHASES_COLLECTION);
  const q = query(colRef, where("listingCreatorId", "==", creatorId));
  const snap = await getDocs(q);
  ...
  for (const d of snap.docs) {
    ...
    purchases.push({
      ...
      listingTitle: listingData?.title,
      listingImage: listingData?.image,
      listingCreatorId: data.listingCreatorId ?? listingData?.creatorId,
    });
  }
  purchases.sort((a, b) => {
    const aPending = a.status === "pending";
